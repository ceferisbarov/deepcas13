...
def train_deepcas13_model(df_train, savepath, basename):
    ...
    for train, test in kf.split(df_train):
        ...
        NN_drop1 = Dropout(0.3)(seq_output)
        ...
        ###
        NN_model.fit(X_train_seq_CNN, 
                     y_train,
                     validation_data = (X_test_seq_CNN, y_test),
                     epochs=15,
                     batch_size=128,
                     shuffle=True,
                     verbose=1)
        train_results.append(NN_model.evaluate(X_train_seq_CNN, y_train))
        test_results.append(NN_model.evaluate(X_test_seq_CNN, y_test))
